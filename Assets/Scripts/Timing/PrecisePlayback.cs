...
namespace NotReaper.Timing
{
    ...
    public class PrecisePlayback : Singleton<PrecisePlayback>
    {
        ...
        {
            int numSamples = (int)(sampleRate * endTime.ToSeconds());
        ...
        void ClickTrackPCMReaderCallback(float[] data)
        {
            ...
            for (int dataIndex = 0; dataIndex < data.Length / 2; dataIndex++)
            {
                ...
                QNT_Timestamp currentTick = QNT_Timestamp.ShiftTick((float)((dataIndex) / (float)sampleRate));
                TempoChange currentTempo = EditorTempo.GetTempoForTime(currentTick);
        ...
        {
            float midTime = time.ToSeconds();
            float duration = (float)Conversion.FromQNT(new Relative_QNT(Math.Abs(previewDuration.tick)), EditorTempo.GetTempoForTime(time).microsecondsPerQuarterNote);
            ...
            if (NRSettings.config.playNoteSoundsWhileScrolling)
            {
                ...
                AddHitsoundEvents(previewHits, time, previewDuration.tick > 0 ? QNT_Timestamp.ShiftTick(time, duration) : time);
        ...
        void AddHitsoundEvents(List<HitsoundEvent> events, QNT_Timestamp start, QNT_Timestamp end)
        {
            ...
            foreach (Target t in new NoteEnumerator(start, end))
            {
                ...
                if (!added)
                {
                    ...
                    //Song End Pitch
                    if (data.time == EditorNotes.OrderedNotes.Last().data.time)
                    {
                        ...
                        if (EditorFile.SongDesc.songEndEvent == "event:/song_end/song_end_nopitch")
                        {
                            continue;
                        }
                        ev.ID = data.ID;
        ...
        void OnAudioFilterRead(float[] bufferData, int bufferChannels)
        {
            ...
            if (song != null)
            {
                QNT_Timestamp timeStart = QNT_Timestamp.ShiftTick(song.CurrentTime);
                QNT_Timestamp timeEnd = QNT_Timestamp.ShiftTick((float)(song.CurrentTime + (bufferData.Length / bufferChannels) / (float)song.frequency * (song.frequency / (float)sampleRate)));
            ...
            for (int dataIndex = 0; dataIndex < bufferData.Length / bufferChannels; dataIndex++)
            {
                ...
                QNT_Timestamp currentTick = QNT_Timestamp.ShiftTick(GetTimeFromCurrentSample());
                TempoChange currentTempo = EditorTempo.GetTempoForTime(currentTick);
                QNT_Duration timeSignatureDuration = new QNT_Duration(Constants.PulsesPerWholeNote / currentTempo.timeSignature.Denominator);
        ...
        {
            QNT_Timestamp nextBeat = EditorTime.GetSnappedTime(currentTick, currentTempo.timeSignature.Denominator);
